...
def generate_pmos_small_signal_model(pmos_name, external_nets_map):
    ...
    if 'D' in external_nets_map and 'S' in external_nets_map:
        parallel_elements = [
            {'type': 'controlled_source', 'expression': gm_expr, 'direction': '<-'},  # Reverse direction
            {'type': 'controlled_source', 'expression': gmb_expr, 'direction': '<-'},  # Reverse direction
            {'type': 'component', 'name': rds_name}
        ]
        model_statements.append({
            'type': 'series_connection',
...
def process_circuit_file(input_file, output_dir=None, stdout=False, debug_dump=False): # Added debug_dump
    ...
    for nmos in nmos_transistors:
        ...
        # Generate model
        model_statements, rule_data = generate_nmos_small_signal_model(nmos, external_nets)
    ...
    for pmos in pmos_transistors:
        ...
        # Generate model
        model_statements, rule_data = generate_pmos_small_signal_model(pmos, external_nets)
        all_model_statements.extend(model_statements)
...
def main():
    ...
    parser.add_argument(
        'circuit_file',
        help='Input circuit file to process'
    )
    parser.add_argument(
        '-o', '--output-dir',
