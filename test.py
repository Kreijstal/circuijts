...
def transform_and_validate_loop(initial_code: str):
    ...
    if not ast_1 and parser_errors:
        ...
        print(
            "Parsing resulted in an empty AST (no errors reported). Continuing cautiously."
        )
    else:
    ...
    print("AST_1 (Initial):")
    for stmt_idx, stmt in enumerate(ast_1):
        print(f"  AST_1 Stmt {stmt_idx}: {stmt}")
    ...
    try:
        ...
        print(
            f"Graph_1 created with {len(graph_1.nodes())} nodes and {len(graph_1.edges())} edges."
        )
        assert graph_1 is not None, "Graph_1 should not be None after ast_to_graph."
        assert (
            len(graph_1.nodes()) > 0 if ast_1 else True
        ), "Graph_1 should have nodes if AST_1 was not empty."
    ...
    print(
        "DSU_1 structure (first 5 items):",
        dict(list(dsu_1.parent.items())[:5]) if dsu_1 else "No DSU object",
    )
    ...
    try:
        ...
        assert (
            ast_2 is not None
        ), "AST_2 should not be None after graph_to_structured_ast."
    except Exception as e:
        print(f"Error during Graph_1 to AST_2 conversion: {e}")
        print(
            "Continuing loop despite AST reconstruction error to allow further steps if possible."
        )

    print("AST_2 (Reconstructed from graph):")
    if ast_2:
        for stmt_idx, stmt in enumerate(ast_2):
            print(f"  AST_2 Stmt {stmt_idx}: {stmt}")
    else:
...
def perform_nmos_ss_transformation_and_flatten(
    initial_circuit_code: str, nmos_to_replace: str
):
    ...
    print(
        f"  Graph_initial: {len(initial_graph.nodes())} nodes, {len(initial_graph.edges())} edges."
    )
    print(f"  Graph_initial nodes: {list(initial_graph.nodes(data=True))}")
    print(
        f"  DSU_initial (first 5 items): {dict(list(initial_dsu.parent.items())[:5])}"
    )

    nmos_external_connections = (
        {}
    )  # Stores terminal_name -> preferred_external_net_name
    if nmos_to_replace in initial_graph:
        ...
        if (
            ...
            print(
                f"  Found NMOS instance '{nmos_to_replace}' with connections to canonical nets:"
            )
            ...
            for terminal, canonical_net_name in term_to_canonical_net_map.items():
                ...
                print(
                    f"    Terminal {terminal} -> Canonical Net '{canonical_net_name}' (Preferred for model: '{preferred_net_name}')"
                )
    ...
    if not required_terminals_for_model.issubset(nmos_external_connections.keys()):
        ...
        print(
            f"Error: Cannot generate model. Missing external connections for terminals: {missing} of {nmos_to_replace}."
        )
        return

    # Generate AST for small-signal model
    print("\n3. Generating AST_ss_model (Small-Signal Model parts)...")
    ss_model_ast_stmts = generate_nmos_small_signal_model_ast(
        nmos_to_replace, nmos_external_connections
    )
    print("AST_ss_model:")
    for i, stmt in enumerate(ss_model_ast_stmts):
        print(f"  [{i:02d}] {stmt}")
    ...
    for stmt in initial_ast:
        if stmt["type"] == "declaration":
            if stmt["instance_name"] == nmos_to_replace:
                continue
    ...
    for stmt in initial_ast:
        if stmt["type"] != "declaration":
            ...
            if (
                ...
                print(
                    f"  Skipping original component_connection_block for {nmos_to_replace}: {stmt}"
                )
                continue
            ...
            if stmt["type"] == "direct_assignment":
                ...
                if stmt.get("source_node", "").startswith(
                    nmos_to_replace + "."
                ) or stmt.get("target_node", "").startswith(nmos_to_replace + "."):
                    is_related_direct_assign = True
                if is_related_direct_assign:
                    print(
                        f"  Skipping original direct assignment stmt related to {nmos_to_replace}: {stmt}"
                    )
                    continue
            ...
            if stmt["type"] == "series_connection":
                ...
                for item in stmt.get("path", []):
                    if (
                        ...
                        and item.get("name") == nmos_to_replace
                    ):
                        is_related_series = True
                        break
                    if item.get("type") == "node" and item.get("name", "").startswith(
                        nmos_to_replace + "."
                    ):
                        is_related_series = True
                        break
                if is_related_series:
                    print(
                        f"  Skipping original series stmt involving {nmos_to_replace} or its terminals: {stmt}"
                    )
                    continue
    ...
    for i, stmt in enumerate(combined_ast_statements):
        print(f"  [{i:02d}] {stmt}")
    ...
    final_transformed_graph, final_transformed_dsu = ast_to_graph(
        combined_ast_statements
    )
    assert (
        final_transformed_graph is not None
    ), "Final transformed graph should not be None."
    print(
        f"  Graph_transformed: {len(final_transformed_graph.nodes())} nodes, {len(final_transformed_graph.edges())} edges."
    )
    print(
        f"  Graph_transformed nodes: {list(final_transformed_graph.nodes(data=True))}"
    )
    print(
        f"  DSU_transformed (first 5 items): {dict(list(final_transformed_dsu.parent.items())[:5])}"
    )
    ...
    final_flattened_ast = ast_to_flattened_ast(
        combined_ast_statements, final_transformed_dsu
    )
    ...
    if not final_flattened_ast:
        ...
        if not regular_ast_from_flattened:
            ...
            reconstructed_code = (
                "; Could not reconstruct regular AST from flattened AST."
            )
        else:
            print(
