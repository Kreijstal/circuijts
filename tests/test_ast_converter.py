...
def test_complex_series_parallel_flattening():
    ...
    try:
        ...
        original_components = {stmt['instance_name'] for stmt in ast
                             if stmt['type'] == 'declaration'}
        flattened_components = {stmt['instance_name'] for stmt in flattened_ast
                              if stmt['type'] == 'declaration'}
        assert original_components == flattened_components
        
        # 2. Verify connection count matches expected topology
        original_connections = sum(1 for stmt in ast
                                 if stmt['type'] in ('connection', 'series', 'parallel'))
        flattened_connections = sum(1 for stmt in flattened_ast
                                  if stmt['type'] == 'pin_connection')
        assert flattened_connections >= original_connections
        
        # 3. Verify no duplicate pin connections
        pin_connections = {(stmt['component_instance'], stmt['terminal'])
                          for stmt in flattened_ast
                          if stmt['type'] == 'pin_connection'}
        assert len(pin_connections) == sum(1 for stmt in flattened_ast
                                        if stmt['type'] == 'pin_connection')
        
        # 4. Verify net aliases are properly handled
        net_aliases = {stmt['source_net']: stmt['canonical_net']
                      for stmt in flattened_ast
                      if stmt['type'] == 'net_alias'}
        for source, canonical in net_aliases.items():
            assert canonical in {stmt['net'] for stmt in flattened_ast
                               if stmt['type'] == 'pin_connection'}
