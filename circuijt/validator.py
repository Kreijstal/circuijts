    ...
    def validate(self):
        ...
        for stmt in self.parsed_statements:
            if stmt["type"] == "declaration":
                ...
                if not re.fullmatch(
                    r"[a-zA-Z_][a-zA-Z0-9_]*", comp_type
                ):  # Parser also checks
                    self._add_error(
                ...
                if not re.fullmatch(
                    r"[a-zA-Z_][a-zA-Z0-9_]*", inst_name
                ):  # Parser also checks
                    self._add_error(
        ...
        for stmt in self.parsed_statements:
            ...
            if stmt_type == "component_connection_block":
                ...
                original_assignments_str = stmt.get(
                    "_original_assignments_str", ""
                ).strip()
                ...
                if not stmt["connections"] and original_assignments_str:
                    ...
                    self._add_error(
                        f"Component block for '{comp_name}' is empty.", line_num
                    )

                # Validate arity for this specific block
                comp_type_from_decl = self.declared_component_types.get(
                    comp_name, {}
                ).get("type")
                ...
                for conn in stmt["connections"]:
                    if not re.fullmatch(
                        r"[a-zA-Z_][a-zA-Z0-9_]*", conn["terminal"]
                    ):  # Parser checks
                        self._add_error(
                ...
                if not path:
                    self._add_error(
                        "Series connection path is empty after parsing.", line_num
                    )
                    continue
